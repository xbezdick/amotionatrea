...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    entry.async_create_background_task(
        hass, coordinator.aatrea.ws_connect(), "amotionatrea-ws_connect"
    )
...
class AtreaWebsocket:
    ...
    async def connect(self, on_data, on_close):
        try:
            async with websockets.connect(
                "ws://%s/api/ws" % self._host, ping_interval=10, ping_timeout=10
            ) as websocket:
                try:
    ...
    async def consumer_handler(self, websocket, on_data):
...
#            self.update()
    ...
    async def send(self,message):
        ...
        try:
            async with asyncio.timeout(TIMEOUT):
                await self._hass.async_add_executor_job(self._websocket.send, json.dumps(msg))
